package com.github.berbatov001.enhancedzuul.netflix.zuul.filters.route;
    ...
    private HttpUriRequest toRequest(final RequestConfig requestConfig, HttpServletRequest request, String serviceName, String originalUri) {
        try {
            ...
            if (requestBody != null) {
                ...
                if (FilterConstants.HTTP_GET_METHOD.equalsIgnoreCase(method) && (contentLength < 0)) {
        ...
        try {
            ...
            if (request instanceof StandardMultipartHttpServletRequest) {
                ...
                for (Map.Entry<String, String[]> entity : parameterMap.entrySet()) {
                    Arrays.stream(entity.getValue()).forEach(value -> multipartBodyBuilder.part(entity.getKey(), value));
                }
                for (Map.Entry<String, List<MultipartFile>> entity : multiFileMap.entrySet()) {
                    entity.getValue().forEach(file -> multipartBodyBuilder.part(entity.getKey(), file.getResource(), MediaType.MULTIPART_FORM_DATA));
                }
